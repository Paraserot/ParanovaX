...
if (!serviceAccount.private_key) {
  console.error("FATAL ERROR: FIREBASE_PRIVATE_KEY environment variable is not set in the workspace.");
} else {
    if (!getApps().some(app => app.name === 'adminDb')) {
        ...
        adminDb = getApps().find(app => app.name === 'adminDb')!;
    }
}
